...
// Set SCL low and drive output (for nRF52840/Arduino)
#define i2c_scl_lo()              \
  pinMode(_sclPin, OUTPUT);       \
  digitalWrite(_sclPin, LOW);

// Release SDA (set as input, enable pullup if needed)
#define i2c_sda_hi()              \
  pinMode(_sdaPin, _pullups ? INPUT_PULLUP : INPUT);

// Release SCL (set as input, enable pullup if needed)
#define i2c_scl_hi()              \
  pinMode(_sclPin, _pullups ? INPUT_PULLUP : INPUT);

// Read the value of SDA pin
#define i2c_sda_read()   (digitalRead(_sdaPin) ? 1 : 0)

// Read the value of SCL pin
#define i2c_scl_read()   (digitalRead(_sclPin) ? 1 : 0)
...
void SoftwareWire::printStatus( Print& Ser)
{
  ...
  Ser.print(F("  _sdaPortReg = "));
  Ser.println((uintptr_t)_sdaPortReg, HEX);
  Ser.print(F("  _sclPortReg = "));
  Ser.println((uintptr_t)_sclPortReg, HEX);
  Ser.print(F("  _sdaDirReg = "));
  Ser.println((uintptr_t)_sdaDirReg, HEX);
  Ser.print(F("  _sclDirReg = "));
  Ser.println((uintptr_t)_sclDirReg, HEX);
  Ser.print(F("  _sdaPinReg = "));
  Ser.println((uintptr_t)_sdaPinReg, HEX);
  Ser.print(F("  _sclPinReg = "));
  Ser.println((uintptr_t)_sclPinReg, HEX);
